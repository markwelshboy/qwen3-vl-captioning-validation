...
if [[ $# -lt 1 ]]; then
  echo "usage: $0 RUN_DIR [support-shape diagnostic options...]" >&2
  exit 2
fi
...
echo "Support-shape diagnostic Python: $PY" >&2
exec "$PY" -m qwen_caption_validate.pose_support_shape_diagnostic "$@"
