...
if [[ $# -lt 1 ]]; then
  echo "Usage: $0 RUN_DIR [training_caption_review_bundle options] [--tar]" >&2
  exit 2
fi
...
"${PY}" -m qwen_caption_validate.training_caption_review_bundle "${ARGS[@]}"

if [[ "${MAKE_TAR}" -eq 1 ]]; then
  RUN_DIR_ABS="$(${PY} -c 'from pathlib import Path; import sys; print(Path(sys.argv[1]).expanduser().resolve())' "${RUN_DIR}")"
  if [[ -z "${OUTPUT}" ]]; then
    OUTPUT="${RUN_DIR_ABS}/semantic-v3/training-caption-review-v0.1"
  else
