...
if [[ $# -lt 1 ]]; then
  echo "usage: $0 RUN_DIR [Phase-4B.11 specialist-normalizer options...]" >&2
  exit 2
fi
...
echo "Phase-4B.11 Python: $PY" >&2
exec "$PY" -m qwen_caption_validate.fact_sheet_specialist_normalizer_v12 "$@"
