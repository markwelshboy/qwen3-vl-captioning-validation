...
echo "Semantic V3 rich/Pose repair v0.4: fail-only repair using editor v0.6 audit and contextual local-attachment validation."
echo "Governed fist laterality still authorizes only the same-side hand; no broader limb-chain inference is added."

cd "$REPO_ROOT"
exec "$PY" -m qwen_caption_validate.semantic_v3_rich_pose_repair_v04 "$@"
