...
if [[ $# -lt 1 ]]; then
  echo "usage: $0 RUN_DIR [Phase-5.1 text-composer options...]" >&2
  exit 2
fi
...
echo "Phase-5.1 Python: $PY" >&2
echo "Phase-5.1 vLLM text-only profile: $QWEN_VLLM_TEXT_ONLY_PROFILE" >&2
exec "$PY" -m qwen_caption_validate.fact_sheet_text_composer_v02 "$@"
