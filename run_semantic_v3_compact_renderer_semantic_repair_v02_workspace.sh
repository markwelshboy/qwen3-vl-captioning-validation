...
echo "Semantic V3 post-budget semantic repair v0.2: deterministic exact-span repair first; Qwen only for unresolved semantic failures."
echo "Length never triggers repair and no image inference occurs."

cd "$REPO_ROOT"
exec "$PY" -m qwen_caption_validate.semantic_v3_compact_renderer_semantic_repair_v02 "$@"
