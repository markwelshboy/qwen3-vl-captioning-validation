...
echo "Semantic V3 rich/Pose repair v0.2: fail-only repair with local grammar/attachment validation."
echo "Governed fist laterality authorizes the same-side hand; no broader limb-chain inference is added."

cd "$REPO_ROOT"
exec "$PY" -m qwen_caption_validate.semantic_v3_rich_pose_repair_v02 "$@"
