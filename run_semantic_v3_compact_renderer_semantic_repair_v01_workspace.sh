...
echo "Semantic V3 post-budget semantic repair v0.1: repair only genuine semantic gate failures."
echo "Length alone never triggers model inference; no image analysis or rich-caption re-expansion occurs."

cd "$REPO_ROOT"
exec "$PY" -m qwen_caption_validate.semantic_v3_compact_renderer_semantic_repair_v01 "$@"
