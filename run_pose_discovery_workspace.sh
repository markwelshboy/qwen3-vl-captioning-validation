...
if [[ ! -x "${QWEN_PY}" ]]; then
  echo "Python environment not found: ${QWEN_PY}" >&2
  exit 2
fi
...
dwpose_dir = Path(sys.argv[2]).resolve()
eligible_path = Path(sys.argv[3]).resolve()
skipped_path = Path(sys.argv[4]).resolve()
...
for image in sorted(
    ...
    try:
        ...
        skipped.append({
            "image": rel.as_posix(),
            "image_key": key,
            "reason": f"invalid_dwpose_json:{type(exc).__name__}",
        })
        continue
    ...
    if not all(name in bbox for name in required):
        skipped.append({
            "image": rel.as_posix(),
            "image_key": key,
            "reason": "missing_dwpose_target_bbox",
        })
        continue
...
skipped_path.write_text(
    json.dumps(
        {
            "schema_version": "pose-discovery-preflight-0.1",
...
if skipped:
    print(f"Skipped detail: {skipped_path}")
PY

mapfile -t ELIGIBLE_IMAGES < "${ELIGIBLE_FILE}"
if [[ "${#ELIGIBLE_IMAGES[@]}" -eq 0 ]]; then
  echo "No images have a usable DWPose target bbox; nothing to run." >&2
  exit 2
fi
...
for image in "${ELIGIBLE_IMAGES[@]}"; do
  SAM_ARGS+=(--include "${image}")
done
