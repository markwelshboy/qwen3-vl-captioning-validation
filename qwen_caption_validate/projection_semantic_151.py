...
from .caption_projection_151 import build_caption_projection
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="qwen-projection-semantic-151",
        description="Replay Projection 1.5.1 from cached Fusion 2.3.x + Pose Semantics v0.10 without loading Compose.",
    )
...
def main() -> int:
    ...
    output_dir = (args.output_dir or (run_dir / "projection-v1.5.1" / slug)).expanduser().resolve()
    ...
    for record in records:
        ...
        economy = integration.get("structural_semantic_economy") or {}
        ...
        index_records.append(
            {
                ...
                "component_pose_fields_removed": economy.get("component_pose_fields_removed") or {},
                "component_required_claims_removed": economy.get("component_required_claims_removed") or [],
            }
        )
        written += 1

    index = {
        "schema_version": "caption-projection-1.5.1-run",
        "run_dir": str(run_dir),
    ...
    index_path = output_dir / "projection_151.index.json"
    _write(index_path, index)
    print(f"Projection 1.5.1 output: {output_dir}")
