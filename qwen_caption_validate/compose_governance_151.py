...
from .caption_projection_151 import build_caption_projection, lint_caption
...
PROJECTION 1.5.1 STRUCTURAL POSE-SEMANTIC GOVERNANCE
...
- If `semantic_pose.posture` is null, do NOT infer standing, seated, squatting, reclining, kneeling, or lying from furniture, crop, scene, or context. Candidate posture is audit-only and absent from caption-facing evidence.
- `pose_orientation.semantic_pose.gestures` contains the caption-preferred recognizable action/support primitives. Express those naturally once.
- Component body-part geometry, raw pose interactions, and legacy gesture bookkeeping have been deliberately removed from caption-facing evidence. Do not reconstruct knee angles, foot placement, arm-chain geometry, hand/forearm duplicates, or support-chain details from the semantic posture or scene.
- Independent information that survives the structural economy boundary may still be described: qualified torso/head orientation, crop/framing, transient clothing/accessories, scene/background objects, and lighting.
- Do not expand `reclining` into supine/prone, bent knees, spread legs, feet-flat, exact body support, or similar subordinate anatomy unless such detail is independently present in the remaining caption-facing evidence.
- Do not expand `seated` into planted feet, exact chair contact, leaning/reclining, or lower-body geometry unless independently present in the remaining caption-facing evidence.
- Continue all Projection 1.4.3 laterality, framing, scene, identity-firewall, and qualified orientation rules.
"""


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="qwen-compose-governance-151",
        description=(
            "Generate text-only captions from Projection 1.5.1, where Pose Semantics v0.10 "
            "is structurally isolated from component pose/support evidence."
        ),
    )
...
def main() -> int:
    ...
    run_label = _base._safe_label(args.run_label or f"semantic151-{args.backend}-{args.quantization}-{args.dtype}")
    ...
    output_dir = run_dir / "compose_semantic_151" / f"{analysis_slug}__compose__{compose_slug}__{run_label}"
    ...
    if pending:
        print(
            f"Loading {compose_model_id} for Projection 1.5.1 semantic Compose "
    ...
    try:
        for item in tqdm(prepared, desc="Semantic governed Compose 1.5.1"):
            if not item["need"]:
                continue
            ...
            metadata = {
                "schema_version": "compose-semantic-1.5.1",
    ...
    summary = {
        "schema_version": "compose-semantic-1.5.1-run",
        "projection_revision": "1.5.1",
        "analysis_model": analysis_model_id,
        "compose_model": compose_model_id,
    ...
    summary_path = output_dir / "compose_semantic_151.index.json"
    summary_path.write_text(json.dumps(summary, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    aggregate_path = output_dir / "compose_semantic_151.txt"
