...
from .caption_projection_156 import lint_caption
...
PROJECTION 1.5.6 NEGATIVE-SPACE SUBJECT-GEOMETRY GOVERNANCE
- The supplied caption evidence has already passed Projection 1.5.6. Treat its FACT semantic pose/orientation and required claims as the caption authority; do not reconstruct discarded low-level anatomy from scene context, framing, or prior conventions.
- `pose_orientation.semantic_pose.posture`, when non-null, is a qualified Pose Semantics v0.10 FACT. State it naturally once. If it is null, do not infer standing, seated, squatting, reclining, kneeling, lying, or an "upright posture" from furniture, crop, framing, or a neutral image-plane axis.
...
- `pose_orientation.subject_geometry_orientation.body_orientation`, when present, is the governed non-frontal body-to-camera yaw relation. Express it naturally and never quote reconstructed degrees.
- ABSENCE of `body_orientation` is authoritative. If that field is absent, do NOT say the body/torso is slightly angled, three-quarter, side-on, turned toward/away from the camera, or facing frame left/right. Do not infer body yaw from photographic framing, face direction, shoulders, or scene context.
- `pose_orientation.subject_geometry_orientation.face_yaw_orientation`, when present, describes HORIZONTAL HEAD/FACE YAW ONLY. It does not describe head pitch and it is not eye gaze. Near-frontal face yaw is deliberately omitted from caption-facing evidence.
- A `head_body_relation` of `turned_toward_camera` is a compound relative-YAW fact: express body direction plus that compensating horizontal head turn as ONE compact relation. This still does not establish eye gaze.
- `pose_orientation.semantic_orientation` contains only non-neutral orientation facts that survived semantic economy. Preserve them when useful. A supported `head_pitch: down` must not be erased or contradicted by yaw wording.
- If `faces_frame` is left/right, it describes the BODY'S direction in the image frame. Do not convert it into anatomical left/right.
- Do not enumerate left/right shoulders, arms, or other visible body parts merely to describe framing or crop. Use anatomical side only when a caption-facing required claim, semantic gesture, or support relation actually needs that laterality. Otherwise use side-neutral wording or omit the body-part enumeration.
- Do not resurrect `signed_shoulder_nearer_relation`, `signed_torso_depth_direction`, raw shoulder depth, root yaw degrees, neutral torso/head axes, or `upright_in_image_plane` bookkeeping removed upstream.
- Subject-relative camera-center geometry remains audit-only. Do not infer high-angle/low-angle photography, selfie capture mode, camera hand, or world camera elevation from it.
- Prefer ordinary human language. Avoid phrases such as "frontal body orientation", "matching orientation", or anatomical visibility checklists.
- Continue the identity firewall, qualified laterality, framing, scene, clothing/accessory, independent gaze, and lighting rules already present in the base prompt/evidence.
"""


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="qwen-compose-governance-156",
        description="Generate text-only captions directly from cached Projection 1.5.6 caption evidence.",
    )
...
def _projection_ok(evidence: dict[str, Any] | None) -> bool:
    return isinstance(evidence, dict) and evidence.get("projection_revision") == "1.5.6"
...
def main() -> int:
    ...
    projection_dir = (
        ...
        else run_dir / "projection-v1.5.6" / analysis_slug
    )
    if not projection_dir.is_dir():
        print(f"Projection 1.5.6 directory not found: {projection_dir}", file=sys.stderr)
        return 2

    compose_model_id = resolve_model_id(args.compose_model)
    compose_slug = model_slug(compose_model_id)
    run_label = _base._safe_label(args.run_label or f"semantic156-{args.backend}-{args.quantization}-{args.dtype}")
    ...
    output_dir = run_dir / "compose_semantic_156" / f"{analysis_slug}__compose__{compose_slug}__{run_label}"
    ...
    for record in image_records:
        ...
        if evidence is None or audit is None:
            missing += 1
            print(f"WARNING: missing Projection 1.5.6 evidence/audit for {key}; skipping", file=sys.stderr)
            continue
        if not _projection_ok(evidence):
            invalid_projection += 1
            print(f"WARNING: {key} caption evidence is not Projection 1.5.6; skipping", file=sys.stderr)
            continue
    ...
    if not prepared:
        print("No complete Projection 1.5.6 records found.", file=sys.stderr)
        return 2
    ...
    if pending:
        print(
            f"Loading {compose_model_id} for Projection 1.5.6 text-only Compose "
    ...
    try:
        for item in tqdm(prepared, desc="Semantic governed Compose 1.5.6"):
            if not item["need"]:
                continue
            ...
            metadata = {
                "schema_version": "compose-semantic-1.5.6",
    ...
    for item in prepared:
        ...
        aggregate_lines.extend(
            [
                ...
                f"semantic_orientation: {json.dumps(semantic_orientation, ensure_ascii=False)}",
    ...
    summary = {
        "schema_version": "compose-semantic-1.5.6-run",
        "projection_revision": "1.5.6",
        "analysis_model": analysis_model_id,
        "compose_model": compose_model_id,
    ...
    summary_path = output_dir / "compose_semantic_156.index.json"
    summary_path.write_text(json.dumps(summary, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    aggregate_path = output_dir / "compose_semantic_156.txt"
