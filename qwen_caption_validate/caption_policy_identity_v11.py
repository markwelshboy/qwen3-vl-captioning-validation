...
DEFAULT_INPUT_SUBDIR = Path("semantic-v3") / "caption-fact-sheet-v0.2.17"
DEFAULT_OUTPUT_SUBDIR = Path("semantic-v3") / "caption-fact-sheet-v0.3.2"
EXPECTED_INPUT_SCHEMA = "caption-fact-sheet-0.2.17"
...
def _apply_identity_policy(source: dict) -> dict:
    ...
    invariants.update(
        phase4b17_capture_style_is_preserved=True,
        mirror_selfie_surface_policy_is_preserved=True,
        direct_selfie_capture_facts_are_preserved=True,
...
def main() -> int:
    ...
    if not input_dir.is_dir():
        print(f"Phase-4B.17 fact-sheet directory not found: {input_dir}", file=sys.stderr)
        return 2
    ...
    for input_path in paths:
        ...
        print(
            f"{key}: {record.get('status')} | identity policy v11 "
    ...
    index = {
        ...
        "capture_subtype_counts": dict(sorted(capture_counts.items())),
        "invariants": {
            "no_model_calls": True,
            "phase4b17_is_input_not_recomputed": True,
            "capture_style_is_preserved": True,
            "mirror_selfie_surface_policy_is_preserved": True,
