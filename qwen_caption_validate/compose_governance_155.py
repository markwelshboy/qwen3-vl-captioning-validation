...
from .caption_projection_155 import lint_caption
...
PROJECTION 1.5.5 YAW-SCOPED SUBJECT-GEOMETRY GOVERNANCE
- The supplied caption evidence has already passed Projection 1.5.5. Treat its FACT semantic pose/orientation and required claims as the caption authority; do not reconstruct discarded low-level anatomy from scene context or prior conventions.
...
- `pose_orientation.subject_geometry_orientation.body_orientation`, when present, is a governed non-frontal body-to-camera yaw relation. Express `slightly_angled`, `three_quarter`, `side_on`, rear-three-quarter, or rear naturally; never quote reconstructed degrees. Frontal body yaw is deliberately omitted because it is usually low-information prose.
- `pose_orientation.subject_geometry_orientation.face_yaw_orientation`, when present, describes HORIZONTAL HEAD/FACE YAW ONLY. It does not describe head pitch and it is not eye gaze. Express the yaw naturally without saying the subject is looking at the camera unless independent gaze evidence explicitly supports that statement.
- Near-frontal face yaw is deliberately omitted from caption-facing evidence. Its absence must NOT be expanded into "facing the camera", "head turned toward the camera", or "looking at the camera".
- A `head_body_relation` of `turned_toward_camera` is a compound relative-YAW fact: the head has turned horizontally toward the camera compared with a substantially turned body. Express body orientation/frame direction plus that relative head turn as ONE compact relation. This still does not establish eye gaze.
- `pose_orientation.semantic_orientation` contains only non-neutral orientation facts that survived semantic economy. Preserve them when useful. In particular, a supported `head_pitch: down` must not be erased or contradicted by a near-frontal yaw interpretation.
...
- Do not resurrect `signed_shoulder_nearer_relation`, `signed_torso_depth_direction`, raw shoulder depth, root yaw degrees, neutral torso/head axes, or `upright_in_image_plane` bookkeeping removed by Projection 1.5.5.
...
- Prefer ordinary human language. Avoid phrases such as "frontal body orientation" or "matching orientation" when no caption-facing claim requires them.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="qwen-compose-governance-155",
        description="Generate text-only captions directly from cached Projection 1.5.5 caption evidence.",
    )
...
def _projection_ok(evidence: dict[str, Any] | None) -> bool:
    return isinstance(evidence, dict) and evidence.get("projection_revision") == "1.5.5"
...
def main() -> int:
    ...
    projection_dir = (
        ...
        else run_dir / "projection-v1.5.5" / analysis_slug
    )
    if not projection_dir.is_dir():
        print(f"Projection 1.5.5 directory not found: {projection_dir}", file=sys.stderr)
        return 2

    compose_model_id = resolve_model_id(args.compose_model)
    compose_slug = model_slug(compose_model_id)
    run_label = _base._safe_label(args.run_label or f"semantic155-{args.backend}-{args.quantization}-{args.dtype}")
    ...
    output_dir = run_dir / "compose_semantic_155" / f"{analysis_slug}__compose__{compose_slug}__{run_label}"
    ...
    for record in image_records:
        ...
        if evidence is None or audit is None:
            missing += 1
            print(f"WARNING: missing Projection 1.5.5 evidence/audit for {key}; skipping", file=sys.stderr)
            continue
        if not _projection_ok(evidence):
            invalid_projection += 1
            print(f"WARNING: {key} caption evidence is not Projection 1.5.5; skipping", file=sys.stderr)
            continue
    ...
    if not prepared:
        print("No complete Projection 1.5.5 records found.", file=sys.stderr)
        return 2
    ...
    if pending:
        print(
            f"Loading {compose_model_id} for Projection 1.5.5 text-only Compose "
    ...
    try:
        for item in tqdm(prepared, desc="Semantic governed Compose 1.5.5"):
            if not item["need"]:
                continue
            ...
            metadata = {
                "schema_version": "compose-semantic-1.5.5",
    ...
    summary = {
        "schema_version": "compose-semantic-1.5.5-run",
        "projection_revision": "1.5.5",
        "analysis_model": analysis_model_id,
        "compose_model": compose_model_id,
    ...
    summary_path = output_dir / "compose_semantic_155.index.json"
    summary_path.write_text(json.dumps(summary, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    aggregate_path = output_dir / "compose_semantic_155.txt"
