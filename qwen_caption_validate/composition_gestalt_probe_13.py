...
DEFAULT_PROMPT = PACKAGE_ROOT / "prompts" / "composition_gestalt_v1_3.txt"
DEFAULT_SCHEMA = PACKAGE_ROOT / "schemas" / "composition_gestalt_v1_3.schema.json"
...
def _govern_output_file(path: Path) -> None:
    ...
    payload["governance_v13"] = {
        "anatomical_laterality_policy": "composition probe is side-neutral; body-part left/right is stripped before downstream use",
...
def main() -> int:
    ...
    output_dir = Path(_option_value(argv, "--output-dir", str(run_dir / "composition-gestalt-v1.3" / slug)) or "").expanduser().resolve()
