...
from .caption_projection_156 import build_caption_projection
...
def main() -> int:
    ...
    output_dir = (args.output_dir or (run_dir / "projection-v1.5.6" / slug)).expanduser().resolve()
    ...
    for record in records:
        ...
        body = orientation.get("body_orientation") if isinstance(orientation, dict) else None
        face_yaw = orientation.get("face_yaw_orientation") if isinstance(orientation, dict) else None
        head = orientation.get("head_body_relation") if isinstance(orientation, dict) else None
        projection155 = ((audit.get("projection") or audit).get("subject_geometry_semantics_155") or {})
        index_records.append(
            {
                ...
                "frontal_body_caption_suppressed": bool(projection155.get("frontal_body_orientation_audit_only")),
                "near_frontal_face_yaw_caption_suppressed": bool(projection155.get("near_frontal_face_yaw_caption_suppressed")),
                "body_orientation_absence_is_hard_negative": not isinstance(body, dict),
    ...
    index = {
        "schema_version": "caption-projection-1.5.6-run",
        "run_dir": str(run_dir),
    ...
    index_path = output_dir / "projection_156.index.json"
    _write(index_path, index)
    print(f"Projection 1.5.6 output: {output_dir}")
    print(
