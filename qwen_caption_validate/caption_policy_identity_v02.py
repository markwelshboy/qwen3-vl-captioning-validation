...
DEFAULT_INPUT_SUBDIR = Path("semantic-v3") / "caption-fact-sheet-v0.2.3"
DEFAULT_OUTPUT_SUBDIR = Path("semantic-v3") / "caption-fact-sheet-v0.3"
EXPECTED_INPUT_SCHEMA = "caption-fact-sheet-0.2.3"
...
def main() -> int:
    ...
    if not input_dir.is_dir():
        print(f"Phase-4B.3 fact-sheet directory not found: {input_dir}", file=sys.stderr)
        return 2
    ...
    index = {
        ...
        "invariants": {
            "no_model_calls": True,
            "phase4b3_is_input_not_recomputed": True,
            "gaze_caption_semantics_are_preserved": True,
