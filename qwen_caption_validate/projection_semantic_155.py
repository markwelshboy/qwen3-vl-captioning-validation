...
from .caption_projection_155 import build_caption_projection
...
def main() -> int:
    ...
    output_dir = (args.output_dir or (run_dir / "projection-v1.5.5" / slug)).expanduser().resolve()
    ...
    if not subject_dir.is_dir():
        print(f"WARNING: Subject Geometry directory not found: {subject_dir}; replay will preserve older orientation behavior", file=sys.stderr)
    ...
    for record in records:
        ...
        v155_audit = ((audit.get("projection") or audit).get("subject_geometry_semantics_155") or {})
        claim_ids = [
            str(item.get("id") or "")
        ...
        residual_legacy = [
            str(item.get("id") or "")
            for item in evidence.get("required_claims") or []
            if isinstance(item, dict) and str(item.get("id") or "") in {
                "signed_shoulder_nearer_relation",
                "signed_torso_depth_direction",
                "subject_geometry_face_orientation",
            }
        ]
        ...
        index_records.append(
            {
                ...
                "face_yaw_orientation": orientation.get("face_yaw_orientation"),
                "head_body_relation": orientation.get("head_body_relation"),
                "subject_geometry_required_claims": claim_ids,
                "residual_legacy_orientation_claims": residual_legacy,
                "cross_source_conflict_count": len(integration.get("cross_source_conflicts_audit_only") or []),
                "neutral_orientation_fields_removed": sorted((v155_audit.get("neutral_semantic_orientation_audit_only") or {}).keys()),
                "retained_semantic_orientation_fields": sorted(semantic_orientation.keys()) if isinstance(semantic_orientation, dict) else [],
                "frontal_body_caption_suppressed": bool(v155_audit.get("frontal_body_orientation_audit_only")),
                "near_frontal_face_yaw_caption_suppressed": bool(v155_audit.get("near_frontal_face_yaw_caption_suppressed")),
    ...
    index = {
        "schema_version": "caption-projection-1.5.5-run",
        "run_dir": str(run_dir),
    ...
    index_path = output_dir / "projection_155.index.json"
    _write(index_path, index)
    print(f"Projection 1.5.5 output: {output_dir}")
    print(
