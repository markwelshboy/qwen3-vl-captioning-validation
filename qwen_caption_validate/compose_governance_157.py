...
from .caption_projection_157 import lint_caption
...
PROJECTION 1.5.7 STRUCTURAL SEMANTIC-ECONOMY GOVERNANCE
- The supplied caption evidence has already passed Projection 1.5.7. Treat its semantic pose/orientation and required claims as the complete caption authority; do not reconstruct removed component geometry or positive-visibility bookkeeping.
- `pose_orientation.semantic_pose.posture`, when non-null, is a qualified Pose Semantics v0.10 FACT. State it naturally once. If it is null, do not infer standing, seated, squatting, reclining, kneeling, lying, or an upright posture from crop, furniture, framing, or scene context.
- `pose_orientation.semantic_pose.gestures` contains caption-preferred action/support primitives. Express them naturally and economically.
- `pose_orientation.subject_geometry_orientation.body_orientation`, when present, is the governed non-frontal body-to-camera yaw relation. Express it naturally; never quote reconstructed degrees. ABSENCE of this field is authoritative: do not invent frontal/near-frontal/slightly-angled/three-quarter/side-on body yaw or frame-facing direction.
- `pose_orientation.subject_geometry_orientation.face_yaw_orientation`, when present, describes HORIZONTAL HEAD/FACE YAW ONLY. It is neither head pitch nor eye gaze. Near-frontal face yaw is intentionally absent from caption-facing evidence.
- A `head_body_relation` of `turned_toward_camera` is a compound relative-yaw fact. Express body direction plus the compensating horizontal head turn as ONE compact relation; do not convert it into eye-gaze language.
- Preserve non-neutral `pose_orientation.semantic_orientation` facts when useful, especially supported head pitch such as `down`.
- Shoulder-girdle, pelvis, and torso depth-rotation measurements are verification ingredients and have been structurally removed once governed body yaw exists. Never regenerate phrases such as "shoulder depth rotation", "shoulder staggering in depth", or similar metric-like geometry.
- `visibility_constraints` now exposes only hard `not_visible` boundaries. Positive/partial landmark visibility is audit-only. Do not enumerate left/right shoulders, arms, hips, legs, or other anatomy merely to demonstrate what is visible in the crop.
- Anatomical side should appear only when a surviving caption-facing semantic gesture, support relation, or other required claim needs that laterality. Otherwise prefer ordinary side-neutral prose.
- Subject-relative camera-center geometry remains audit-only. Do not infer selfie mode, camera hand, or world high/low angle from it.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="qwen-compose-governance-157",
        description="Generate text-only captions directly from cached Projection 1.5.7 caption evidence.",
    )
...
def _projection_ok(evidence: dict[str, Any] | None) -> bool:
    return isinstance(evidence, dict) and evidence.get("projection_revision") == "1.5.7"
...
def main() -> int:
    ...
    projection_dir = (
        ...
        else run_dir / "projection-v1.5.7" / analysis_slug
    )
    if not projection_dir.is_dir():
        print(f"Projection 1.5.7 directory not found: {projection_dir}", file=sys.stderr)
        return 2

    compose_model_id = resolve_model_id(args.compose_model)
    compose_slug = model_slug(compose_model_id)
    run_label = _base._safe_label(args.run_label or f"semantic157-{args.backend}-{args.quantization}-{args.dtype}")
    ...
    output_dir = run_dir / "compose_semantic_157" / f"{analysis_slug}__compose__{compose_slug}__{run_label}"
    ...
    for record in image_records:
        ...
        if evidence is None or audit is None:
            missing += 1
            print(f"WARNING: missing Projection 1.5.7 evidence/audit for {key}; skipping", file=sys.stderr)
            continue
        if not _projection_ok(evidence):
            invalid_projection += 1
            print(f"WARNING: {key} caption evidence is not Projection 1.5.7; skipping", file=sys.stderr)
            continue
    ...
    if not prepared:
        print("No complete Projection 1.5.7 records found.", file=sys.stderr)
        return 2
    ...
    if pending:
        print(
            f"Loading {compose_model_id} for Projection 1.5.7 text-only Compose "
    ...
    try:
        for item in tqdm(prepared, desc="Semantic governed Compose 1.5.7"):
            if not item["need"]:
                continue
            ...
            metadata = {
                "schema_version": "compose-semantic-1.5.7",
    ...
    summary = {
        "schema_version": "compose-semantic-1.5.7-run",
        "projection_revision": "1.5.7",
        "analysis_model": analysis_model_id,
        "compose_model": compose_model_id,
    ...
    summary_path = output_dir / "compose_semantic_157.index.json"
    summary_path.write_text(json.dumps(summary, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    aggregate_path = output_dir / "compose_semantic_157.txt"
