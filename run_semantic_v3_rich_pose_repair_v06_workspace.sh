...
echo "Semantic V3 rich/Pose repair v0.6: v0.5 semantic policy plus local haircut-residue grammar validation."
echo "Only failed captions are regenerated; primary/secondary identity policy and Pose authority remain unchanged."

cd "$REPO_ROOT"
exec "$PY" -m qwen_caption_validate.semantic_v3_rich_pose_repair_v06 "$@"
